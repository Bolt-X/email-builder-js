...
import { Box, Tabs, Tab, Button, Typography, IconButton, Stack, ToggleButton } from '@mui/material';
...
export default function ImageSidebarPanel({ data, setData }: ImageSidebarPanelProps) {
  ...
  const handleUpload = async (file: File) => {
    ...
    const url = assetURL + res?.data?.data?.id;
  ...
  return (
    <BaseSidebarPanel title="Image block">
      ...
      >
        <Tab icon={<CloudUploadIcon fontSize="small" />} iconPosition="start" label="Upload" />
      ...
      {tab === 0 && (
        ...
        >
          {!data.props?.url ? (
            ...
            <Box>
              <img src={data.props.url} alt="preview" style={{ maxWidth: '100%', marginBottom: 8 }} />
              <IconButton color="error" onClick={() => updateData({ ...data, props: { ...data.props, url: null } })}>
                <DeleteIcon />
              </IconButton>
            </Box>
          )}
